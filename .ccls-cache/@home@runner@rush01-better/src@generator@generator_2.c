...
static void generate_constraints(t_map *map)
{
	...
	while (i < map->size)
	{
		map->constraints[i] = map_visible_top(map, i, NULL);
		i++;
	}
	i = 0;
	while (i < map->size)
	{
		map->constraints[map->size + i] = map_visible_bottom(map, i, NULL);
		i++;
	}
	i = 0;
	while (i < map->size)
	{
		map->constraints[2 * map->size + i] = map_visible_left(map, i, NULL);
		i++;
	}
	i = 0;
	while (i < map->size)
	{
		map->constraints[3 * map->size + i] = map_visible_right(map, i, NULL);
		i++;
	}
}
